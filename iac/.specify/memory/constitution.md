# ShoppingList IaC Constitution

## Core Principles

### I. Reproducible Infrastructure First

All infrastructure changes MUST be declarative, reviewable, and reproducible from source control.

### II. Environment Isolation and Explicit Inputs

Environment-specific behavior MUST be modeled through explicit configuration and modules, not by ad-hoc edits or hidden defaults.

### III. Validate Before Apply (NON-NEGOTIABLE)

Every change MUST follow a validation-first workflow: lint or format, static validation, plan or preview, then apply in a controlled context.

### IV. Security, Least Privilege, and Observability

Infrastructure changes MUST define identity boundaries, secret handling, access scope, logging, and operational visibility as part of the feature.

### V. Safe Rollout and Drift Awareness

Features MUST define rollback or mitigation strategy, expected blast radius, and how drift or out-of-band changes are detected.

## Project Structure & Quality Gates

- Shared modules live under `modules/` when the chosen stack supports modular composition
- Environment definitions live under `environments/` or another explicit folder defined in the plan
- Specs live under `specs/`
- Plans MUST define the selected IaC toolchain before implementation begins
