# ShoppingList Web Constitution

## Core Principles

### I. Accessible Web Experience First

Features MUST be keyboard-usable, screen-reader-friendly, responsive, and explicit about loading, error, and empty states.

### II. Typed Contract-First Integration

All backend interactions MUST align with documented API contracts. Request and response shapes MUST be represented explicitly in frontend code.

### III. Test-First Development (NON-NEGOTIABLE)

Every feature MUST follow Red-Green-Refactor, with unit coverage for logic and component behavior plus integration or end-to-end coverage for critical flows.

### IV. Progressive Enhancement and Resilience

Features MUST tolerate latency, retries, partial failure, and auth/session edge cases without trapping users in broken states.

### V. Explicit Frontend Architecture

Each feature plan MUST document the chosen framework, routing approach, state boundaries, and rendering strategy instead of relying on implicit defaults.

## Project Structure & Quality Gates

- Source code lives under `src/`
- Tests live under `test/` or `e2e/`
- Specs live under `specs/`
- Plans MUST define the chosen web stack before implementation begins
